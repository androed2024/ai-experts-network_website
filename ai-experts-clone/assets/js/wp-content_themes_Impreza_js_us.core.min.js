...
options=$.extend({},$.fn.horparallax.defaults,typeof options=='object'&&options);this.options=options;this.bg=this.container.find(options.bgSelector);this.containerWidth=this.container.outerWidth();this.containerHeight=this.container.outerHeight();this.bgWidth=this.bg.outerWidth();this.windowHeight=this.$window.height();this._frameRate=Math.round(1000/this.options.fps);if(!('ontouchstart' in window)||!('DeviceOrientationEvent' in window)){this.container.on('mouseenter.noPreventDefault',(e)=>{var offset=that.container.offset(),coord=(e.pageX-offset.left)/that.containerWidth;that.cancel();that._hoverAnimation=!0;that._hoverFrom=that.now;that._hoverTo=coord;that.start(that._hoverTo)}).on('mousemove.noPreventDefault',(e)=>{if(that._lastFrame+that._frameRate>Date.now()){return}
var offset=that.container.offset(),coord=(e.pageX-offset.left)/that.containerWidth;if(that._hoverAnimation){that._hoverTo=coord;return}
that.set(coord);that._lastFrame=Date.now()}).on('mouseleave.noPreventDefault',()=>{that.cancel();that.start(that.options.basePoint)})}
...
var $item=$(e.target).closest('.menu-item'),$target=$(e.target);$item.parents('.menu-item').addClass('opened');$item.on('mouseleave.noPreventDefault',()=>{$target.blur()})}.bind(this),blurHandler:function(e){if(this.type=='mobile'){return}
...
this.$container.appendTo("body");$('body').not(this.activeArea).on('mouseup.noPreventDefault',function(){var selection=this.getSelection();if(selection===''){this.$container.hide()}}.bind(this));$(this.activeArea).on('mouseup.noPreventDefault',function(e){var selection=this.getSelection();if(selection!==''){this.selectedText=selection;this.showTooltip(e)}else{this.selectedText='';this.hideTooltip()}}.bind(this));this.$copy2clipboard.on('click',function(){this.copyToClipboard()}.bind(this))}},showTooltip:function(e){$('.w-sharing-item',this.$container).each(function(index,elm){if($(elm).hasClass('copy2clipboard')){return}
...
const self=this;self._updateCart();let $notification=self.$cartNotification,$productName=$('.product-name',$notification),productName=$productName.text();productName=$button.closest('.product').find('.woocommerce-loop-product__title').text();$productName.html(productName);$notification.addClass('shown');$notification.on('mouseenter.noPreventDefault',()=>{$notification.removeClass('shown')});$ush.timeout(()=>{$notification.removeClass('shown').off('mouseenter')},3000)},_outsideClickEvent:function(e){const self=this;if($.contains(self.$cart[0],e.target)){return}
